...
EntryPoint::EntryPoint(EntryPoint::EntryPointKind kind, llvm::Function *function, Module *module)
  ...
  std::accumulate(
      ...
      d_arguments,
      [=](auto argument, auto& function_argument) -> auto {
  ...
  unsigned index = 0;
  std::transform(
      d_arguments, d_arguments + d_argument_count,
...
EntryPoint::EntryPoint(EntryPoint::EntryPointKind kind, llvm::MDNode *md, Module *module)
  ...
  , d_module(module) {
  d_function_md.reset(
    llvm::cast<llvm::ValueAsMetadata>(md->getOperand(0).get()));
  ...
  // Arguments:
  d_arguments_md.reset(
      llvm::cast<llvm::MDTuple>(d_md->getOperand(2).get()));
  ...
  unsigned index = 0;
  std::transform(
      d_arguments_md->op_begin(), d_arguments_md->op_end(),
  ...
  std::sort(
      arguments_md.begin(), arguments_md.end(),
      [](auto lhs, auto rhs) -> bool {
	  return
	      llvm::mdconst::extract<llvm::ConstantInt>(lhs.first->getOperand(0).get())->getZExtValue() <
	      llvm::mdconst::extract<llvm::ConstantInt>(rhs.first->getOperand(0).get())->getZExtValue();
      });

  // Construct d_arguments:
  struct Compare {
      bool operator()(llvm::Argument *lhs, std::pair<llvm::MDTuple *, unsigned > rhs) const {
	  return lhs->getArgNo() < llvm::mdconst::extract<llvm::ConstantInt>(rhs.first->getOperand(0).get())->getZExtValue();
      }
      
      bool operator()(std::pair<llvm::MDTuple *, unsigned> lhs, llvm::Argument *rhs) const {
	  return llvm::mdconst::extract<llvm::ConstantInt>(lhs.first->getOperand(0).get())->getZExtValue() < rhs->getArgNo();
      }
  };
...
EntryPoint::setModule(Module *module) {
  ...
  if (d_module) {
	...
	if (entry_points_md) {
	    auto it = std::find(
		entry_points_md->op_begin(), entry_points_md->op_end(), d_md.get());
	    if (it != entry_points_md->op_end()) {
		entry_points_md->setOperand(
		    std::distance(entry_points_md->op_begin(), it),
		    nullptr);
	    }
	}
    }
  ...
  if (d_module) {
    if (function) {
	LLAIRContextImpl::Get(d_module->getContext()).entry_points().insert(std::make_pair(function, this));
    }
...
{
  return d_function_md
      ? llvm::cast<llvm::Function>(
	  d_function_md->getValue())
      : nullptr;
}
...
EntryPoint::updateArgumentMetadata(Argument *argument) {
    d_arguments_md->replaceOperandWith(
	argument->d_md_index,
	argument->d_md.get());
}
    ...
    , d_parent(entry_point)
{
	...
	llvm::MDTuple::get(
		...
		llvm::ConstantAsMetadata::get(
		    llvm::ConstantInt::get(ll_context, llvm::APInt(32, d_function_argument->getArgNo(), true))),
...
EntryPoint::Argument::Argument(llvm::Argument *function_argument, llvm::MDTuple *md, EntryPoint *entry_point)
    : d_function_argument(function_argument)
    , d_parent(entry_point)
{
    d_md.reset(md);
	...
	if (string == "air.arg_name") {
	...
	else if (string == "air.buffer" || string == "air.indirect_buffer" || string == "air.texture") {
		...
		for (auto it1 = d_md->op_begin(), it1_end = d_md->op_end(); it1 != it1_end; ) {
			...
			auto buffer_size_md = llvm::mdconst::dyn_extract<llvm::ConstantInt>(it1++->get());
			if (!buffer_size_md) {
			    continue;
			}
			...
			auto type_size_md = llvm::mdconst::dyn_extract<llvm::ConstantInt>(it1++->get());
			if (!type_size_md) {
			    continue;
			}
			...
			auto type_align_size_md = llvm::mdconst::dyn_extract<llvm::ConstantInt>(it1++->get());
		...
		if (string == "air.buffer") {
		    d_details = Buffer({ *location0, *location1, *access, *type_size, *type_align_size });
		}
		else if (string == "air.indirect_buffer") {
		    d_details = IndirectBuffer({ *location0, *location1, *access, *type_size, *type_align_size });
		}
	    }
	    ...
	    EntryPoint::Argument::Interpolation interpolation = EntryPoint::Argument::Interpolation::kCenterPerspective;
		...
		if (interpolation1 == "air.center" || interpolation1 == "air.centroid" || interpolation1 == "air.sample") {
		    string_md = llvm::dyn_cast<llvm::MDString>(it1++->get());
		    if (!string_md) {
			continue;
		    }
			...
			if (interpolation2 == "air.no_perspective") {
			    interpolation = EntryPoint::Argument::Interpolation::kCenterNoPerspective;
			}
			...
			if (interpolation2 == "air.no_perspective") {
			    interpolation = EntryPoint::Argument::Interpolation::kCentroidNoPerspective;
			}
			else if (interpolation2 == "air.perspective") {
			    interpolation = EntryPoint::Argument::Interpolation::kCentroidPerspective;
			}
		    }
		    else if (interpolation1 == "air.sample") {
			if (interpolation2 == "air.no_perspective") {
			    interpolation = EntryPoint::Argument::Interpolation::kSampleNoPerspective;
			}
			else if (interpolation2 == "air.perspective") {
...
EntryPoint::Argument::InitDetailsAsBuffer(const Buffer& buffer) {
	...
	llvm::MDTuple::get(ll_context, {
		...
		buffer.access == Access::kReadOnly
		    ? llvm::MDString::get(ll_context, "air.read")
...
void
EntryPoint::Argument::InitDetailsAsIndirectBuffer(unsigned location0, unsigned location1, Access access) {
    auto type = llvm::cast<llvm::PointerType>(d_function_argument->getType())->getElementType();
...
EntryPoint::Argument::InitDetailsAsTexture(const Texture& texture) {
	...
	llvm::MDTuple::get(ll_context, {
		...
		texture.access == Access::kReadOnly
		    ? llvm::MDString::get(ll_context, "air.read")
		    : llvm::MDString::get(ll_context, "air.read_write"),
		llvm::MDString::get(ll_context, "air.arg_type_name"),
		llvm::MDString::get(ll_context, getTypeName()),
...
EntryPoint::Argument::getName() const {
    ...
    return llvm::cast<llvm::MDString>(d_md->getOperand(*d_name_md_index).get())->
	getString();
}
...
EntryPoint::Argument::getTypeName() const {
    ...
    return llvm::cast<llvm::MDString>(d_md->getOperand(*d_type_name_md_index).get())->
	getString();
}
...
EntryPoint::Argument::setTypeName(llvm::StringRef type_name) {
    ...
    d_md->replaceOperandWith(
	*d_type_name_md_index,
	llvm::MDString::get(d_parent->getModule()->getLLContext(), type_name));
}

// VertexEntryPoint
VertexEntryPoint * VertexEntryPoint::Create(llvm::Function *function, unsigned output_count, Module *module) {
...
: EntryPoint(EntryPoint::Vertex, function, module) {
  ...
  std::accumulate(
      d_outputs, d_outputs+d_output_count,
      d_outputs,
      [=](auto output, auto&) -> auto {
...
, d_outputs(nullptr) {
  ...
  std::accumulate(
      d_outputs_md->op_begin(), d_outputs_md->op_end(),
      d_outputs,
      [=](auto output, auto& operand) -> auto {
...
VertexEntryPoint::updateOutputMetadata(Output *output) {
    d_outputs_md->replaceOperandWith(
	output->d_md_index,
	output->d_md.get());
    ...
    : d_parent(entry_point)
{
	...
	if (string == "air.arg_name") {
	    ...
	    // Determine interpolation:
	    VertexEntryPoint::Output::Interpolation interpolation = VertexEntryPoint::Output::Interpolation::kCenterPerspective;
		...
		if (interpolation1 == "air.center" || interpolation1 == "air.centroid" || interpolation1 == "air.sample") {
		    ...
		    if (interpolation1 == "air.center") {
			if (interpolation2 == "air.no_perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kCenterNoPerspective;
			}
			else if (interpolation2 == "air.perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kCenterPerspective;
			}
		    }
		    else if (interpolation1 == "air.centroid") {
			if (interpolation2 == "air.no_perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kCentroidNoPerspective;
			}
			else if (interpolation2 == "air.perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kCentroidPerspective;
			}
		    }
		    else if (interpolation1 == "air.sample") {
			if (interpolation2 == "air.no_perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kSampleNoPerspective;
			}
			else if (interpolation2 == "air.perspective") {
			    interpolation = VertexEntryPoint::Output::Interpolation::kSamplePerspective;
			}
		    }
		}
		else if (interpolation1 == "air.flat") {
...
FragmentEntryPoint::Output::InitDetailsAsRenderTarget(const RenderTarget& render_target) {
	...
	llvm::MDTuple::get(ll_context, {
		...
		llvm::ConstantAsMetadata::get(
		    llvm::ConstantInt::get(ll_context, llvm::APInt(32, render_target.location0, true))),
		llvm::ConstantAsMetadata::get(
		    llvm::ConstantInt::get(ll_context, llvm::APInt(32, render_target.location1, true))),
		llvm::MDString::get(ll_context, "air.arg_type_name"),
		llvm::MDString::get(ll_context, getTypeName()),
		llvm::MDString::get(ll_context, "air.arg_name"),
		llvm::MDString::get(ll_context, getName())
