...
class Namespace {
public:
    using Node = std::variant<std::unique_ptr<Class>, std::unique_ptr<Namespace>,
...
main(int argc, char **argv) {
    ...
    std::for_each(input_modules.begin(), input_modules.end(),
    ...
    std::set_difference(
        declared_functions.begin(), declared_functions.end(), defined_functions.begin(),
        defined_functions.end(), std::back_inserter(undefined_functions),
        [](auto lhs, auto rhs) -> bool { return lhs->getName().compare(rhs->getName()) < 0; });

    std::for_each(undefined_functions.begin(), undefined_functions.end(),
                  [](auto function) -> void {
                      ...
                      // std::cerr << std::get<0>(*names).str() << " " << std::get<1>(*names).str()
                      // << std::endl;
                  });
